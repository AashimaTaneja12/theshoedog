...
/*====================mycode to get user id(changes only in 2 place)===========================*/
 ...
 /*===========================End my code ===========================*/  
   ...
   // global $wpdb;
    ...
    $data = $user_id."-".$pid."-".$email;

    $button_code .= '<form target="'.$target.'" action="'.$action_url.'" method="post" >';
    $button_code .= '<input type="hidden" name="cmd" value="_xclick">';
    $button_code .= '<input type="hidden" name="return" value="http://localhost/wordpresstest/wait/">';
   //$button_code .= '<input type="hidden" name="custom" value="'.$user_id.'">';
    /*$button_code .= '<input type="hidden" name="custom" value="'.$user_id.'-'.$pid.'-'.$email.'">';*/ 
    $button_code .= '<input type="hidden" name="custom" value="'.$data.'">';// my one line code
    $paypal_email = get_option('wp_paypal_email');
    if(isset($paypal_email) && !empty($paypal_email)) {
        $button_code .= '<input type="hidden" name="business" value="'.$paypal_email.'">';
    }
    ...
    $button_code .= '<input type="image" class="buy_now" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTZ0X6nFYDIqOAx02slU3CfOZfmXydQziKWyagnhh3rksY7c2IBnQ" border="0" name="submit">';
    $button_code .= '</form>';
    return $button_code;        
}
