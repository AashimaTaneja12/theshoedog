...
$paypal_username = 'sonu179208@gmail.com';*/ //Business Email
...
?>
		...
		function confirm_my_bid() {
			...
			var bid_am 			= bid_am1.formatMoney(2, point_point, comma_comma);
			...
			bid_fee_ens = '';
			var total = parseInt(bid_am)+100;

			var mess = '<table width="100%" class="sitemile-table">'+
			'<tr>'+
			'<td><?php _e('Bid Details','AuctionTheme'); ?></td>'+
			'</tr>'+
			'<tr>'+
			'<td><?php _e('Confirm your bid details','AuctionTheme'); ?></td>'+
			'</tr>'+
			'<tr>'+
			'<td><?php _e(AuctionTheme_get_first_post_image(get_the_ID(), 0,0 ),'AuctionTheme'); ?></td>'+
			'<td><?php _e(the_title(),'AuctionTheme');?></td>'+
			'</tr>'+
			'<tr>'+
			...
			'</tr>'+
			'<tr>'+
			'<td><?php _e('Estimated Shipping:','AuctionTheme'); ?></td>'+
			'<td>'+ 'R100' + '</td>' +
			'</tr>'+
			'<tr>'+
			'<td><?php _e('Authentication Fee:','AuctionTheme'); ?></td>'+
			'<td>'+ 'Free' + '</td>' +
			'</tr>'+
			'<tr>'+
			'<td><?php _e('Total:','AuctionTheme'); ?></td>'+
	...
	if(isset($_POST['buy_now']))
	{
		...
		elseif(isset($_POST['control_id']))
		{
			...
			//	wp_redirect(get_bloginfo('siteurl')."/?a_action=buy_now_commit&pid=".$pid);
				wp_redirect(get_bloginfo('siteurl').'/buy-now?pid='.$pid);
				exit;
			...
			if($closed == "0"):
				...
				//-------------------------
					...
					elseif($bid_posted != "0"):
						...
						//====================highest value store in this query of particular bid  ( 1 query to insert value in auction bid table)===============

						$select_user_exist1 = $wpdb->get_results("SELECT * from wp_auction_bids where uid = '$uid' && pid = '$pid'" );
						$row_count1 = $wpdb->num_rows;
					
						if($row_count1 == 1)
						{
						//$errors['error'] = __("Your bid has not been posted. ","AuctionTheme");
							$bid_posted = "0";
						...
						AuctionTheme_send_email_when_bid_item_bidder($pid, $uid, $bid);  //send email to bidder
						//AuctionTheme_send_email_when_bid_item_owner($pid, $uid, $bid);
					}
				...
				elseif($reverse != "yes"):
					/*===================== 2nd query to insert value in auction table===============*/
					global $wpdb,$post;

					$select_user_exist = $wpdb->get_results("SELECT * FROM wp_auction_bids WHERE uid=$uid && pid=$pid");

					$row_count = $wpdb->num_rows;
					
					if($row_count == 1)
					{
						//$errors['higher_bid'] = __("Your bid has not been posted. Please correct the errors and try again.","AuctionTheme");
						$bid_posted = "0";
					}
					else{
					...
					/*================================3rd query to insert value in auction bid table(only 3 query)==========*/
				elseif($reverse != "no"):
					...
					$bid_posted = "0";
						...
						wp_redirect(get_permalink(get_the_ID()) . "/?bid_posted=1");  //any user store the lowest bid from current bid then store
						}
...
<div class="my_box3">
	...
	?>
			...
			?>
 			...
 			<div class="my_box3">
            	<div class="box_title auction_page_title Product_text"><h1><?php the_title() ?></h1></div>
					<div class="col-xs-12 col-sm-4 col-lg-4 bb-image bb_margin">
						...
						<!-- ========================Modal to display all bids=======================-->
						<?php 
						//echo $pid;
						$query = $wpdb->get_results("SELECT * from wp_auction_bids where pid = '$pid' ORDER BY bid DESC");
						
						$display_bid = $wpdb->get_results("SELECT * from wp_auction_bids where pid = '$pid'");
						
						$highest_bid = $wpdb->get_results("SELECT * from wp_highest_bid where pid = '$pid'");
						global $wp;
						$url = home_url( $wp->request );
						if(!empty($highest_bid)){
							$highbid = $highest_bid[0]->highest_bid;
						
				
						?>
					
						<a href="<?php echo $url;?>"><button class="highbid">R<?php echo $highbid;?><br/>Highest Bid | Sell or Ask</button></a>
						<?php
						}
						else{?>
							<a href="<?php echo $url;?>"><button><?php echo 'No highest bid';?><br/>Highest Bid | Sell or Ask</button></a>
							<?php
						}
						?>
						
						<br/><a href="#" data-toggle="modal" class="Bids" data-target="#modalTC">View all Bids</a>
							   ...
							   <!-- Modal content-->
							    <div class="modal-content">
							     <div class="modal-header">
							     	<h4 class="modal-title">All Bids
							      <button type="button" class="close text-dark" data-dismiss="modal">&times; </button> </h4>
							    ...
							    <div class="modal-footer">
							    	<button type="button" class="d-block btn text-white close_btn_float" data-dismiss="modal">Close</button>
							    </div>
							   </div>
							  </div>
							 </div>
			...
			<!--=======================End  to display modal==================-->
				...
				<div class="col-xs-12 col-sm-8 col-lg-8 bb_margin bb_marginmm">
                ...
                <form method="post"> <input type="hidden" name="control_id" value="<?php echo  (get_the_ID()); ?>" />
                   ...
                   <?php endif; ?>
                            ...
                            <li>
								...
								<p><?php echo "R".auctionTheme_get_buy_it_now_price(get_the_ID()); ?></p> 
							</li>
              ...
              <?php
                ...
                <div class="padd10">
					...
					if(!empty($winner))
					{
						...
						_e("Product closed : ","AuctionTheme");
                ...
                <div class="clear10"></div>
                        ...
                        <div class="row border-bottom bottom_padding">
								<div class="col-xs-5 col-sm-5 col-lg-5 font-weight-bold"><i class="fab fa-elementor"></i> <?php echo __("Product ID","AuctionTheme"); ?>:</div>
           ...
           <!-- </div>
			...
			<div class="my_box3">
				...
				/*===================Custom code to insert high and low value in custom table=========================*/
				
				$high1 = "select MAX(bid) as max,MIN(bid) as min from ".$wpdb->prefix."auction_bids where pid = '$pid'";
				$highres  = $wpdb->get_results($high1);
				
				$max_bid = $highres[0]->max;
				$min_bid = $highres[0]->min;
				$select_pid = "select * from ".$wpdb->prefix."highest_bid where pid = '$pid' ";
				$select_pidres  = $wpdb->get_row($select_pid);
				if($select_pidres == 1)
				{
					//echo "here";
					$maxupdate 	= "update ".$wpdb->prefix."highest_bid set highest_bid='$max_bid',lowest_bid='$min_bid' where pid='$pid'";
					$wpdb->query($maxupdate);
				}
				
				else{
					//echo "2";
				$highlow_insert = "insert into ".$wpdb->prefix."highest_bid (pid,highest_bid,lowest_bid) values('$pid','$max_bid','$min_bid')";
				$wpdb->query($highlow_insert);
				}
				/*==================================End Custom==============================*/
			...
			<div class="my_box3">
				...
				foreach($arr as $image)
				{
					echo '<li class="product_img"><a href="'.AuctionTheme_wp_get_attachment_image($image, array(900, 700)).'" rel="image_gal2">'.wp_get_attachment_image( $image, array(200, 180) ).'</a></li>';
				}
				echo '</ul>';
...
<?php
	...
	echo '<div id="right-sidebar" class="page-sidebar col-xs-12 col-sm-4 col-lg-4 sidebar_border">';
	echo '<ul class="xoxo">';
...
<?php } ?>
	...
	<li class="widget-container widget_text margin_top aaad-other-details" id="ad-other-details">
